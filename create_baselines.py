...
def create_baseline_models(num_models=5, num_steps=1000):
    ...
    for i in range(num_models):
        print(f"\nTraining model {i+1}/{num_models} with architecture {factories[i].hidden_layers}")
        agent = DQNAgent(
            env=env,
            dqn_factory=factories[i],
            gamma=0.99,
            epsilon_start=0.1,
            epsilon_decay=0.995,
            epsilon_end=0.01,
        ...
        for step in tqdm.tqdm(range(num_steps), desc="Training"):
            ...
            for j in range(env_params['n_drones']):
                agent.learn(state[j], actions[j], rewards[j], next_state[j], dones[j])
        ...
        # Save model
        save_path = os.path.join(os.path.dirname(__file__), 'baseline_models', f'dqn-agent-{i+1}.safetensors')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-models', type=int, default=5, help='Number of baseline models to create')
    parser.add_argument('--num-steps', type=int, default=1000, help='Number of training steps per model')
